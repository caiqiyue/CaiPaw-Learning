...
> 这是 L7 唯一能落到「事后验证」的地方 —— `docs/` 不入库，没有别的检查能看到它。

- [ ] 本次拆的模块是 `M0N`，`docs/进度/M0N-*.md` **§2 已填**
- [ ] §2.2 的每一问都给了**源码行号**（不只是函数名）
- [ ] **§3 里每一步引用的符号，都在 §2.2 出现过** ← 最硬的一条
- [ ] 脚手架行数 ≤ 2，且都是**显式**写的「——（无对应，工程脚手架）」，不是留空
- [ ] §2.3「我们简化 / 换掉了什么」**不是空表**（空 = 要么真没改，要么没意识到自己在改）

> 勾不上 → **回去补 §2，或者承认这轮拆得不算数**。
> 这五条不是走形式：L7 是**唯一落到防线 2 的硬约束**（见 harness §4.3.1），
> 没有 Hook、没有自动化能替你兜底。
